...
| Роль        | Возможности                                         |
| `ORGANIZER` | Полное управление поездкой, приглашение участников, |
|             | редактирование маршрута и бюджета, удаление поездки |
...
userId пользователя добавляется в массив participants у поездки;
пользователь добавляется в подколлекцию participants внутри поездки;
поездка появляется в списке поездок приглашённого пользователя.
Логика маршрута
...
пользователь формирует маршрут в приложении;
нажимает кнопку открытия маршрута в Яндекс Картах;
приложение передаёт координаты точек во внешнее приложение;
Яндекс Карты строят маршрут по дорожной сети.
Настройка проекта
...
добавить google-services.json в папку app;
добавить MAPKIT_API_KEY в local.properties;
...
Открывает созданную поездку.
Переходит во вкладку маршрута.
Выполняет поиск места.
Добавляет найденное место в маршрут.
Добавляет ещё несколько точек маршрута.
Меняет порядок точек маршрута.
Открывает вкладку карты.
Проверяет отображение маркеров и линии маршрута.
Экспортирует маршрут в Яндекс Карты.
Переходит во вкладку бюджета.
Добавляет расходы.
Проверяет общую сумму расходов.
Переходит во вкладку участников.
Приглашает второго пользователя по email.
Второй пользователь входит в приложение.
Второй пользователь принимает приглашение.
Поездка появляется у второго пользователя.
Пользователь просматривает уведомления.
Пользователь открывает профиль.
Организатор удаляет поездку.
